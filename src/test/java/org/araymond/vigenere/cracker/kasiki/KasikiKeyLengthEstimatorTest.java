...
public class KasikiKeyLengthEstimatorTest {
    ...
    @Test
    public void shouldEstimateKeyLengthWithShortText() {
        ...
        final String encoded = vigenere.cypher(shortStory, key);
        assertThat(estimator.estimate(encoded)).extracting("length").has(new Condition<List<?>>() {
            @Override
            public boolean matches(List<?> value) {
                for (Integer val : (List<Integer>) value) {
                    if (val % key.length() == 0) {
                        return true;
                    }
                }
    ...
    @Test
    public void shouldEstimateKeyLengthWithMedium() {
        ...
        final String encoded = vigenere.cypher(mediumStory, key);
