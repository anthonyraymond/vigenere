 ...
 */
public class BazeriesKeyFinderTest {

    private final VigenereCipher vigenere = new VigenereCipher();
    private final BazeriesKeyFinder bazeries = new BazeriesKeyFinder();

    @Test
    public void shouldFindKeyEvenIfLastWord() {
        final String plaintext = "between the puzzle and her son, Lippman says she initially regarded the offer with skepticism";
        final String key = "ours";

        final String cipher = vigenere.cypher(plaintext, key);
        assertThat(bazeries.findPossibleKeys(cipher, "skepticism")).contains(key);
    }
    ...
    public void shouldFindKeyWithExtraLongText() {
        ...
        final String cipher = vigenere.cypher(longStory, key);
        assertThat(bazeries.findPossibleKeys(cipher, "happiness")).contains(key);
    }
